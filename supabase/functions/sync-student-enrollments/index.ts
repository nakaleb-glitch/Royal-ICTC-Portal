...
const getHomeroom = (classValue: unknown) => String(classValue || '').trim().split(/\s+/)[0] || ''
...
serve(async (req) => {
  ...
  try {
    ...
    if (callerProfile?.role !== 'admin' && callerProfile?.role !== 'admin_teacher') {
      return new Response(JSON.stringify({ error: 'Only admins can sync student enrollments' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const academicYear = String(body?.academic_year || '').trim() || '2026-2027'

    if (students.length === 0) {
      return new Response(JSON.stringify({ enrolled: 0, missing: 0, missingStudents: [] }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (classError) {
      return new Response(JSON.stringify({ error: `Class lookup failed: ${classError.message}` }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (studentIds.length > 0 && classIds.length > 0) {
      ...
      if (deleteError) {
        return new Response(JSON.stringify({ error: `Enrollment cleanup failed: ${deleteError.message}` }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
    ...
    if (enrollRows.length > 0) {
      ...
      if (enrollError) {
        return new Response(JSON.stringify({ error: `Enrollment upsert failed: ${enrollError.message}` }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
    ...
    return new Response(JSON.stringify({ enrolled, missing: missingStudents.length, missingStudents }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  } catch (err) {
