...
export default function StudentGradebookLookup() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Load grades for all classes in parallel
                  ...
                  <div className="overflow-x-auto">
                    <div className="min-w-max">
                      ...
                      <div className="flex bg-gray-50 border-b border-gray-200">
                        ...
                        <div style={{ width: '80px', minWidth: '80px', height: '44px', fontSize: '12px', lineHeight: '16px', whiteSpace: 'nowrap' }} className="text-center px-2 font-medium bg-gray-200 text-gray-700 flex items-center justify-center">
                          Grade
                        </div>
                        
                        {attributeNames.map(attr => (
                          <div key={attr} style={{ width: '80px', minWidth: '80px', height: '44px', fontSize: '12px', lineHeight: '16px', whiteSpace: 'nowrap' }} className="text-center px-2 font-medium bg-blue-100 text-blue-800 flex items-center justify-center">
                            {attr.charAt(0).toUpperCase() + attr.slice(1)}
                          </div>
                        ))}

                        {(selectedTerm === 'final_1' || selectedTerm === 'final_2') && (
                          <div style={{ width: '180px', minWidth: '180px', height: '44px', fontSize: '12px', lineHeight: '16px', whiteSpace: 'nowrap' }} className="text-center px-2 font-medium bg-gray-200 text-gray-700 flex items-center justify-center">
                            Teacher Comment
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex border-b border-gray-100">
                        <div style={{ width: '100px', minWidth: '100px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-gray-50 flex items-center justify-center">
                          <span className="text-black font-sans">{fmt(subjectGradeCache[selectedSubject].participation)}</span>
                        </div>
                        <div style={{ width: '130px', minWidth: '130px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-gray-50 flex items-center justify-center">
                          <span className="text-black font-sans">{fmt(subjectGradeCache[selectedSubject].attainment)}</span>
                        </div>
                        <div style={{ width: '100px', minWidth: '100px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-green-50 flex items-center justify-center">
                          <span className="text-black font-sans font-bold">{fmt(subjectGradeCache[selectedSubject].calculatedAttainment)}</span>
                        </div>
                        
                        {classes.find(c => c.id === selectedSubject)?.subject === 'ESL' ? (
                          <>
                            <div style={{ width: '100px', minWidth: '100px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-gray-50 flex items-center justify-center">
                              <span className="text-black font-sans">{fmt(subjectGradeCache[selectedSubject].progressTestRW)}</span>
                            </div>
                            <div style={{ width: '100px', minWidth: '100px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-gray-50 flex items-center justify-center">
                              <span className="text-black font-sans">{fmt(subjectGradeCache[selectedSubject].progressTestListening)}</span>
                            </div>
                            <div style={{ width: '100px', minWidth: '100px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-gray-50 flex items-center justify-center">
                              <span className="text-black font-sans">{fmt(subjectGradeCache[selectedSubject].progressTestSpeaking)}</span>
                            </div>
                            <div style={{ width: '100px', minWidth: '100px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-green-50 flex items-center justify-center">
                              <span className="text-black font-sans font-bold">{fmt(subjectGradeCache[selectedSubject].progressTest)}</span>
                            </div>
                          </>
                        ) : (
                          <div style={{ width: '100px', minWidth: '100px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-green-50 flex items-center justify-center">
                            <span className="text-black font-sans">{fmt(subjectGradeCache[selectedSubject].progressTest)}</span>
                          </div>
                        )}
                        
                        <div style={{ width: '100px', minWidth: '100px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-gray-50 flex items-center justify-center">
                          <span className="text-black font-sans font-bold">{fmt(subjectGradeCache[selectedSubject].overall)}</span>
                        </div>
                        <div style={{ width: '80px', minWidth: '80px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-gray-50 flex items-center justify-center">
                          <span className={`inline-block w-8 h-8 leading-8 rounded font-bold text-sm ${
                            subjectGradeCache[selectedSubject].letterGrade === 'A*' || subjectGradeCache[selectedSubject].letterGrade === 'A' ? 'bg-green-100 text-green-700'
                            : subjectGradeCache[selectedSubject].letterGrade === 'B' ? 'bg-blue-100 text-blue-700'
                            : subjectGradeCache[selectedSubject].letterGrade === 'C' ? 'bg-amber-100 text-amber-700'
                        ...
                        {attributeNames.map(attr => (
                          <div key={attr} style={{ width: '80px', minWidth: '80px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-blue-50 flex items-center justify-center">
                            <span className="text-black font-sans">{subjectGradeCache[selectedSubject].attributes?.[attr] != null ? fmt(subjectGradeCache[selectedSubject].attributes[attr]) : '—'}</span>
                          </div>
                        ))}

                        {(selectedTerm === 'final_1' || selectedTerm === 'final_2') && (
                          <div style={{ width: '180px', minWidth: '180px', height: '52px', fontSize: '12px' }} className="px-2 text-center bg-gray-50 flex items-center justify-center cursor-pointer hover:bg-gray-100" onClick={() => subjectGradeCache[selectedSubject].comment && setSelectedComment(subjectGradeCache[selectedSubject].comment)}>
                            <span className="text-black font-sans truncate">
                              {subjectGradeCache[selectedSubject].comment ? subjectGradeCache[selectedSubject].comment.substring(0, 35) + (subjectGradeCache[selectedSubject].comment.length > 35 ? '...' : '') : '—'}
                            </span>
                          </div>
                        )}
                      </div>
