...
export default function Resources() {
  ...
  const [filter, setFilter] = useState({ level: null, programme: null, subject: null })
  ...
  const filtered = resources.filter(r =>
    (filter.level === null || r.level === filter.level) &&
    (filter.programme === null || r.programme === filter.programme) &&
    (filter.subject === null || r.subject === filter.subject)
  )
  ...
  const filterBtn = (active, onClick, label, activeColor, darkText = false) => (
    <button onClick={onClick}
      className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${
        active ? 'border-transparent' : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50'
      }`}
      style={active ? { backgroundColor: activeColor, color: darkText ? '#1a1a1a' : '#fff' } : {}}>
      {label}
    </button>
  )
                  ...
                  <td className="px-4 py-3">
                    {editingId === r.id ? (
                      <div className="flex gap-2">
                        <button onClick={() => saveEdit(r.id)} className="px-3 py-1 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700">Save</button>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => startEdit(r)} className="px-3 py-1 border border-gray-300 text-gray-600 rounded-lg text-xs hover:bg-gray-50">Edit</button>
                        <button onClick={() => deleteResource(r.id)} className="px-3 py-1 border border-red-200 text-red-500 rounded-lg text-xs hover:bg-red-50">Delete</button>
                      </div>
                    )}
                  </td>
                </tr>
